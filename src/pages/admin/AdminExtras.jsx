...
export default function AdminExtras() {
  ...
  const loadDailyReport = async (date) => {
    try {
      ...
      setMessage({ type: 'error', text: 'Failed to load daily transaction report' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrintReport = () => {
    ...
    if (!printWindow || !dailyReport) return;
    
    const formattedDate = new Date(dailyReport.date + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    
    printWindow.document.write(`
      <html>
      <head>
        <title>Daily Transaction Report - ${formattedDate}</title>
        <style>
          body { font-family: Arial, sans-serif; padding: 20px; color: #333; }
          h1 { font-size: 20px; margin-bottom: 4px; }
          h2 { font-size: 16px; color: #666; margin-bottom: 20px; }
          .summary { display: flex; gap: 20px; margin-bottom: 20px; }
          .summary-box { border: 1px solid #ddd; padding: 12px 16px; border-radius: 6px; }
      ...
      <body>
        ...
        <div class="summary">
          <div class="summary-box">
            ...
            <div class="value">$${dailyReport.total_collected.toFixed(2)}</div>
          </div>
          <div class="summary-box">
            <div class="label">Transactions</div>
            <div class="value">${dailyReport.transaction_count}</div>
          </div>
        </div>
        ${dailyReport.method_breakdown.length > 0 ? `
          <h3 style="font-size:14px; margin-bottom:8px;">Payment Methods</h3>
          <table style="width:auto; margin-bottom:20px;">
            <tr><th>Method</th><th class="right">Amount</th></tr>
            ${dailyReport.method_breakdown.map(m => `<tr><td>${m.method}</td><td class="right">$${m.total.toFixed(2)}</td></tr>`).join('')}
          </table>
        ` : ''}
        <h3 style="font-size:14px; margin-bottom:8px;">Transaction Details</h3>
        ...
        <div class="footer">Generated by MySchoolHub &bull; Printed on ${new Date().toLocaleString()}</div>
      </body>
      </html>
    `);
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <button
              onClick={() => { setActiveSection('daily_report'); loadDailyReport(); }}
              className={`p-6 rounded-xl border-2 transition-all text-left ${
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1 text-sm">
                    View daily payment transaction reports with totals, breakdowns by payment method, and printable summaries.
                  </p>
                  <div className="mt-3 flex items-center text-xs text-gray-500">
        ...
        {activeSection === 'daily_report' && (
          <div className="bg-white rounded-xl shadow-lg p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">Daily Transaction Report</h3>
              <div className="flex items-center gap-3">
                <button
                  onClick={handlePrintReport}
                  disabled={!dailyReport || dailyReport.transaction_count === 0}
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-orange-50 rounded-lg p-4">
                    ...
                    <p className="text-2xl font-bold text-orange-700">${dailyReport.total_collected.toFixed(2)}</p>
                  </div>
                  <div className="bg-blue-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600">Transactions</p>
                    <p className="text-2xl font-bold text-blue-700">{dailyReport.transaction_count}</p>
                  </div>
                  <div className="bg-green-50 rounded-lg p-4">
                    <p className="text-sm text-gray-600">Report Date</p>
                    <p className="text-2xl font-bold text-green-700">
                      {new Date(dailyReport.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </p>
                  </div>
                </div>
                ...
                {dailyReport.method_breakdown.length > 0 && (
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {dailyReport.method_breakdown.map((m, i) => (
                        <div key={i} className="bg-gray-50 border rounded-lg px-4 py-3 flex items-center gap-3">
                          <span className="text-sm text-gray-600">{m.method}</span>
                          <span className="text-sm font-bold text-gray-900">${m.total.toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="overflow-x-auto">
                  {dailyReport.transactions.length === 0 ? (
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {dailyReport.transactions.map((txn) => (
                          <tr key={txn.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 text-sm font-medium">{txn.payment_time}</td>
                            <td className="px-4 py-3">
                              <div className="font-medium">{txn.student_name}</div>
                              {txn.student_number && (
                                <div className="text-xs text-gray-500">{txn.student_number}</div>
                              )}
                            </td>
                            <td className="px-4 py-3 text-sm">{txn.class_name}</td>
                            <td className="px-4 py-3">
                              <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                                {txn.payment_method}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-sm text-gray-600">{txn.transaction_reference || '-'}</td>
                            <td className="px-4 py-3 text-right font-bold text-green-600">${txn.amount.toFixed(2)}</td>
                            <td className="px-4 py-3 text-sm text-gray-600">{txn.processed_by}</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot className="bg-orange-50">
                        <tr>
                          ...
                          <td className="px-4 py-3 text-right font-bold text-orange-700 text-lg">${dailyReport.total_collected.toFixed(2)}</td>
                          <td></td>
                        </tr>
                      </tfoot>
                    </table>
